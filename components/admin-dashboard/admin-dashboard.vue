...
<template>
	<template v-if="busInfoError || getRidesError">
		<div class="flex flex-col items-center w-full bg-warning-300 text-error font-semibold py-3">
			...
			<UButton label="Pokušajte ponovo" class="cursor-pointer mt-4" @click="() => $router.go(0)" />
		</div>
	</template>
	...
	<template v-else>
		<div class="flex items-center flex-col gap-10 lg:gap-30 w-full">
			...
			<div v-if="busesLoading || getRidesLoading"
			...
			<div v-else
				...
				<div v-for="route in routesWithRides" :key="route.id">
					<UTree v-model:expanded="expandedRoutes" :items="getTreeItems(route)"
						...
						<template
							...
							<UTooltip :content="{ side: 'top' }" :delay-duration="0" :ui="{
								content: 'bg-primary',
							}">
								<div class="flex flex-col gap-2 w-full">
									<div
										...
										<Icon name="lucide:bus" size="20" />

										<p>
											{{ item.bus.name }}
										</p>
										<p>Slobodnih mesta - {{
											item.bus.freeSeats }}</p>

										<div v-if="buses && item.bus && item.bus.busId"
											class="flex gap-1">
						...
						<template
							#ride-item="{ item }: { item: { label: string, icon: string, orphans: Orphan[], rideId: string, buses: Bus[] } }">
							<AdminDashboardRideItem :icon="item.icon" :label="item.label"
								:orphans="item.orphans" :buses="item.buses"
								:ride-id="item.rideId" :expanded-routes="expandedRoutes"
								@orphan-assigned="refetchGetRides" />
						</template>

						<template #add-bus-item="{ item }: { item: { rideId: string } }">
							<AdminDashboardAddBusSelector v-if="buses" :buses="buses"
								:buses-loading="busesLoading" :ride-id="item.rideId"
